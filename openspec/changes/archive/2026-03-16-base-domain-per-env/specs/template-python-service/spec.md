...
The system SHALL provide `templates/python-service/cookiecutter.json` with `project_name`, `project_slug`, `description`, `github_org`, `service_type` (external/internal), `service_port`, `docker_registry`, `base_domain_dev`, `base_domain_test`, `base_domain_prod`, and `memory_limit`. The system SHALL NOT include `base_domain` or `traefik_host`.
